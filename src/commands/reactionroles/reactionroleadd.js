...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Add an emoji-role pair to a reaction role')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addIntegerOption(opt => opt.setName('id').setDescription('Reaction role ID').setRequired(true))
    .addStringOption(opt => opt.setName('emoji').setDescription('The emoji to use').setRequired(true))
  ...
  async execute(interaction, args) {
    ...
    try {
      ...
      return interaction.reply({ embeds: [successEmbed('Role Added', `Added ${emoji} → ${role} but couldn't react to the message.`)] });
    }

    return interaction.reply({ embeds: [successEmbed('Role Added', `Added ${emoji} → ${role} to reaction role #${id}.`)] });
  }
};
