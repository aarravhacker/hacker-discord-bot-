...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('reason').setDescription('Reason for the deafen'))
  ...
  async execute(interaction, args) {
    ...
    if (!target) return interaction.reply({ embeds: [errorEmbed('Please provide a valid user to deafen.')] });
    ...
    try {
      ...
      if (guildConfig?.mod_log_channel) {
        ...
        if (logChannel) {
          logChannel.send({
            embeds: [successEmbed(`**Deafened** ${target.user.tag} (${target.id})\n**Moderator:** ${member.user.tag}\n**Reason:** ${reason}\n**Case:** #${caseNumber}`)]
          });
        }
      }

      return interaction.reply({ embeds: [successEmbed(`Successfully deafened ${target.user.tag}`)] });
    } catch (err) {
      console.error(err);
      return interaction.reply({ embeds: [errorEmbed('An error occurred while trying to deafen the user.')] });
    }
  }
};
