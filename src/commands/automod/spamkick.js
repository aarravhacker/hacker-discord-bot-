...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('spamkick')
    .setDescription('Set spam action to kick')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  cooldown: 3,
  aliases: ['setspamkick', 'spkick'],
  prefix: true,
  async execute(interaction, args) {
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('Spam action set to **kick**.')
      ...
      console.error(err);
      await interaction.reply({ embeds: [errorEmbed('Failed to set spam kick.')] });
    }
  }
};
