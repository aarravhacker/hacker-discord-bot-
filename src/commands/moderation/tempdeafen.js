...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('tempdeafen')
    .setDescription('Temporarily deafen a member')
    .addUserOption(option => option.setName('user').setDescription('The user to tempdeafen').setRequired(true))
    .addStringOption(option => option.setName('duration').setDescription('Duration (e.g., 1h, 1d)').setRequired(true))
    .addStringOption(option => option.setName('reason').setDescription('Reason for the tempdeafen'))
  ...
  async execute(interaction, args) {
    ...
    if (!target) return interaction.reply({ embeds: [errorEmbed('Please provide a valid user to tempdeafen.')] });
    ...
    try {
      ...
      await target.send({ embeds: [errorEmbed(`You have been temporarily deafened in **${interaction.guild.name}** for ${durationStr}. Reason: ${reason}`)] }).catch(() => {});

      setTimeout(async () => {
        try {
          const memberNow = interaction.guild.members.cache.get(target.id) || await interaction.guild.members.fetch(target.id);
          if (memberNow.voice.channel) {
            await memberNow.voice.setDeaf(false, 'Tempdeafen duration expired');
            ...
            if (guildConfig?.mod_log_channel) {
              ...
              if (logChannel) {
                logChannel.send({ embeds: [successEmbed(`**Auto-undeafened** ${target.user.tag} (${target.id}) - Tempdeafen expired`)] });
              }
            }
          }
        } catch (e) {
          console.error('Failed to auto-undeafen:', e);
        }
      }, duration);
      ...
      if (guildConfig?.mod_log_channel) {
        ...
        if (logChannel) {
          logChannel.send({
            embeds: [successEmbed(`**Tempdeafened** ${target.user.tag} (${target.id}) for ${durationStr}\n**Moderator:** ${member.user.tag}\n**Reason:** ${reason}\n**Case:** #${caseNumber}`)]
          });
        }
      }

      return interaction.reply({ embeds: [successEmbed(`Successfully tempdeafened ${target.user.tag} for ${durationStr}`)] });
    } catch (err) {
      console.error(err);
      return interaction.reply({ embeds: [errorEmbed('An error occurred while trying to tempdeafen the user.')] });
    }
  }
};
