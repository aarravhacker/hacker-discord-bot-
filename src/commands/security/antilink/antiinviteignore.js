...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antiinviteignore')
    .setDescription('Add or remove a user from antiinvite ignore list')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      let ignoreList = JSON.parse(guildData.antilink_config || '{}').inviteIgnore || [];
      ...
      if (isSlash) {
        ...
        if (args.length < 2) {
          return interaction.reply({ embeds: [warningEmbed('Warning', 'Usage: antiinviteignore <add|remove> <user>')] });
        }
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to update antiinvite ignore list.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
