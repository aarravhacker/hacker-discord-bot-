...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antilinklog')
    .setDescription('View antilink security logs')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (logs.length === 0) {
        return interaction.reply({ embeds: [infoEmbed('Antilink Logs', 'No antilink logs found.')] });
      }
      ...
      const embed = successEmbed('Antilink Logs', logEntries);
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to fetch antilink logs.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
