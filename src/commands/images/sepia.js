...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('sepia')
    .setDescription('Apply sepia filter to an image')
    .addAttachmentOption(opt => opt.setName('image').setDescription('Image to apply sepia').setRequired(true)),
  cooldown: 5,
  aliases: [],
  prefix: true,
  async execute(interaction, args) {
    try {
      ...
      console.error(err);
      await interaction.reply({ embeds: [errorEmbed('Failed to apply sepia.')] });
    }
  }
};
