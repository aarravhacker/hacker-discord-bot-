...
module.exports = {
  ...
  async execute(interaction, args) {
    ...
    if (targetMember.roles.cache.has(role.id)) {
      const embed = new EmbedBuilder()
        .setColor(0xffa500)
        .setDescription(`${targetMember} already has the VIP role.`);
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }
    ...
    const embed = new EmbedBuilder()
      .setColor(0x00ff00)
      .setDescription(`Assigned the VIP role to ${targetMember}.`);
