...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('purgebot')
    .setDescription('Purge bot messages from a channel')
  ...
  async execute(interaction, args) {
    ...
    try {
      ...
      await addModLog({
        ...
        action: 'purgebot',
        reason: `Purged ${deleted.size} bot messages in ${channel.name}`,
        case_number: caseNumber
      });

      return interaction.reply({ embeds: [successEmbed(`Successfully deleted ${deleted.size} bot messages.`)] });
    } catch (err) {
      console.error(err);
      return interaction.reply({ embeds: [errorEmbed('An error occurred while purging bot messages.')] });
    }
  }
};
