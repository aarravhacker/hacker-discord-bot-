...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antibotreport')
    .setDescription('Bot activity report')
  ...
  async execute(interaction, args) {
    ...
    const report = await securityEngine.generateBotReport(interaction.guild.id, period);

    const embed = new EmbedBuilder()
      .setTitle('Bot Activity Report')
      .setDescription(`Bot report for **${interaction.guild.name}** (${period})`)
      .setColor(0x0099ff)
      .addFields(
        { name: 'Total Bots Scanned', value: `\`${report.totalBots || 0}\``, inline: true },
        { name: 'Suspicious Bots', value: `\`${report.suspiciousBots || 0}\``, inline: true },
        { name: 'Bots Removed', value: `\`${report.botsRemoved || 0}\``, inline: true },
        { name: 'Bots Blocked', value: `\`${report.botsBlocked || 0}\``, inline: true },
        { name: 'Verified Bots', value: `\`${report.verifiedBots || 0}\``, inline: true },
        { name: 'Unverified Bots', value: `\`${report.unverifiedBots || 0}\``, inline: true }
      );
