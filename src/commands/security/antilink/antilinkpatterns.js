...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antilinkpatterns')
    .setDescription('View link patterns')
  ...
  async execute(interaction, args) {
    ...
    const patternsData = await securityEngine.getLinkPatterns(interaction.guild.id);
    const patterns = patternsData.patterns || [];
    const totalDetections = patternsData.totalDetections || 0;
    const lastDetection = patternsData.lastDetection;

    const embed = new EmbedBuilder()
      .setTitle('Link Patterns')
      .setDescription(`Link patterns detected in **${interaction.guild.name}**`)
      .setColor(0xff8800)
      .addFields(
    ...
    if (patterns.length > 0) {
      ...
      embed.addFields({ name: 'Patterns', value: 'No link patterns currently detected.' });
    }
