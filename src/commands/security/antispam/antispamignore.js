...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Add or remove a user from antispam ignore list')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      let ignoreList = JSON.parse(guildData.antispam_config || '{}').ignore || [];
      ...
      if (isSlash) {
        ...
        if (args.length < 2) {
          return interaction.reply({ embeds: [warningEmbed('Warning', 'Usage: antispamignore <add|remove> <user>')] });
        }
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to update antispam ignore list.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
