...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antilinkignore')
    .setDescription('Add or remove a user from antilink ignore list')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      let ignoreList = JSON.parse(guildData.antilink_config || '{}').ignore || [];
      ...
      if (isSlash) {
        ...
        if (args.length < 2) {
          return interaction.reply({ embeds: [warningEmbed('Warning', 'Usage: antilinkignore <add|remove> <user>')] });
        }
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to update antilink ignore list.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
