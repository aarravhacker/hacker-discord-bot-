...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antiraidpatterns')
    .setDescription('View detected raid patterns')
  ...
  async execute(interaction, args) {
    ...
    const patternsData = await securityEngine.getRaidPatterns(interaction.guild.id);
    const patterns = patternsData.patterns || [];
    const totalDetections = patternsData.totalDetections || 0;
    const lastDetection = patternsData.lastDetection;

    const embed = new EmbedBuilder()
      .setTitle('Detected Raid Patterns')
      .setDescription(`Raid patterns detected in **${interaction.guild.name}**`)
      .setColor(0xff8800)
      .addFields(
      ...
      patterns.slice(0, 10).forEach((pattern, i) => {
        ...
        embed.addFields({
          ...
          value: [
            ...
            `**Accounts:** ${pattern.accountsInvolved || 0}`
          ].join('\n')
        });
      });
    } else {
      embed.addFields({ name: 'Patterns', value: 'No raid patterns currently detected.' });
    }
