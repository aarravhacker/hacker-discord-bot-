...
module.exports = {
  ...
  async execute(interaction, args) {
    ...
    const role = interaction.guild.roles.cache.find(r => r.name === 'Friend');
    if (!role) {
      const embed = new EmbedBuilder()
        .setColor(0xff0000)
        .setDescription('The "Friend" role does not exist in this server.');
    ...
    if (!targetMember.roles.cache.has(role.id)) {
      const embed = new EmbedBuilder()
        .setColor(0xffa500)
        .setDescription(`${targetMember} does not have the Friend role.`);
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }
    ...
    const embed = new EmbedBuilder()
      .setColor(0x00ff00)
      .setDescription(`Removed the Friend role from ${targetMember}.`);
