...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antibothealth')
    .setDescription('Bot protection health check')
  ...
  async execute(interaction, args) {
    ...
    const health = await securityEngine.getBotProtectionHealth(interaction.guild.id);
    ...
    const embed = new EmbedBuilder()
      .setTitle('Bot Protection Health Check')
    ...
    if (components.detection) {
      const status = components.detection.enabled ? '✅ Enabled' : '❌ Disabled';
      embed.addFields({ name: 'Bot Detection', value: status, inline: true });
    }

    if (components.verification) {
      const status = components.verification.enabled ? '✅ Enabled' : '❌ Disabled';
      embed.addFields({ name: 'Verification System', value: status, inline: true });
    }

    if (components.autoModeration) {
      const status = components.autoModeration.enabled ? '✅ Enabled' : '❌ Disabled';
      embed.addFields({ name: 'Auto Moderation', value: status, inline: true });
    }
