...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('lockdownlog')
    .setDescription('View lockdown logs')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (logs.length === 0) {
        return interaction.reply({ embeds: [infoEmbed('Lockdown Logs', 'No lockdown logs found.')] });
      }
      ...
      const embed = successEmbed('Lockdown Logs', logEntries);
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to fetch lockdown logs.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
