...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antispamwarn')
    .setDescription('Warn a user for spamming')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addUserOption(opt =>
      opt.setName('user').setDescription('User to warn').setRequired(true)
    )
    .addStringOption(opt =>
      opt.setName('reason').setDescription('Warning reason')
    ),
  cooldown: 5,
  aliases: ['aswarn'],
  prefix: true,
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (isSlash) {
        ...
        if (args.length === 0) {
          return interaction.reply({ embeds: [warningEmbed('Warning', 'Please mention a user to warn.')] });
        }
      ...
      const embed = successEmbed(
        'Antispam Warning',
        `⚠️ **${targetUser.tag}** has been warned for spamming.\n**Reason:** ${reason}`
      );
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to warn user for spamming.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
