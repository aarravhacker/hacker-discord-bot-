...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antispammute')
    .setDescription('Mute a user for spamming')
    ...
    try {
      ...
      if (!muteRole) {
        muteRole = await guild.roles.create({
          name: 'Muted',
          permissions: [],
          reason: 'Created for antispam muting'
        });
      ...
      await member.roles.add(muteRole, `${reason} | Muted by ${moderator.tag} via antispam`);
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to mute user for spamming.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
