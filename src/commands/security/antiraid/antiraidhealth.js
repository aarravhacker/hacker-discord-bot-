...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antiraidhealth')
    .setDescription('Check raid protection health')
  ...
  async execute(interaction, args) {
    ...
    const health = await securityEngine.getRaidProtectionHealth(interaction.guild.id);
    ...
    const embed = new EmbedBuilder()
      .setTitle('Raid Protection Health Check')
    ...
    if (components.raidDetection) {
      const status = components.raidDetection.enabled ? '✅ Enabled' : '❌ Disabled';
      embed.addFields({ name: 'Raid Detection', value: status, inline: true });
    }
    ...
    if (components.raidPolicy) {
      embed.addFields({ name: 'Raid Policy', value: `\`${components.raidPolicy.active || 'alert'}\``, inline: true });
    }
