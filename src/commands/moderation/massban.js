...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('reason').setDescription('Reason for the mass ban'))
  ...
  async execute(interaction, args) {
    ...
    if (userIds.length > 50) return interaction.reply({ embeds: [errorEmbed('You can only mass ban up to 50 users at a time.')] });
    ...
    if (guildConfig?.mod_log_channel) {
      ...
      if (logChannel) {
        logChannel.send({
          embeds: [successEmbed(`**Mass Ban** by ${member.user.tag}\n**Banned:** ${successCount}\n**Failed:** ${failCount}\n**Reason:** ${reason}`)]
        });
      }
    }
