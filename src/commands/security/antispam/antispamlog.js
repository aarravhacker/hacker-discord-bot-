...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antispamlog')
    .setDescription('View antispam security logs')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const logs = await getSecurityLogs(guild.id, 'antispam', Math.min(limit, 25));

      if (logs.length === 0) {
        return interaction.reply({ embeds: [infoEmbed('Antispam Logs', 'No antispam logs found.')] });
      }
      ...
      const embed = successEmbed('Antispam Logs', logEntries);
      ...
      console.error(error);
      const embed = errorEmbed('Error', 'Failed to fetch antispam logs.');
      if (isSlash) {
        await interaction.reply({ embeds: [embed] });
      } else {
        await interaction.reply({ embeds: [embed] });
